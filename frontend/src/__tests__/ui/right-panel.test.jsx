...
describe('RightPanel', () => {
  ...
  await userEvent.click(await screen.findByRole('tab', { name: /Controls/i }))
    ...
    const flipYCb = screen.getByLabelText(/Flip Y/i)
    const calCb = screen.getByLabelText(/Calibrate/i)
    const miniCb = screen.getByLabelText(/Show mini map/i)
    await userEvent.click(showDebugCb)
    await userEvent.click(flipYCb)
    await userEvent.click(calCb)
    await userEvent.click(miniCb)
    expect(setShowDebug).toHaveBeenCalled()
    expect(setInvertY).toHaveBeenCalled()
    expect(setCalMode).toHaveBeenCalled()
