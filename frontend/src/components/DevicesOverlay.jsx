  ...
  return (
    <>
      {(employees||[]).map(emp=> {
        ...
        return (
          <div key={emp.id} className="overlay-device" style={{ position:'absolute', left, top, zIndex:5 }}>
            <Mantine.Popover width={220} position="right" withArrow>
              <Mantine.Popover.Target>
                <Mantine.Badge color="red" radius="xl" variant="filled" style={{ cursor:'pointer' }}>{deviceNames[emp.id]||emp.label||emp.id}</Mantine.Badge>
              </Mantine.Popover.Target>
