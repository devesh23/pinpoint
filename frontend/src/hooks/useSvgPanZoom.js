...
export function useSvgPanZoom({ planRef, imgRef, image, disabled = false }) {
  ...
  // Helpers that rely on current DOM metrics
  const clientToUnits = useMemo(() => {
    return function clientToUnits(clientX, clientY) {
      if (!planRef.current) return { x: clientX, y: clientY }
      const planRect = planRef.current.getBoundingClientRect()
      const wrapper = imgRef.current
      if (!wrapper) return { x: (clientX - planRect.left), y: (clientY - planRect.top) }
      let svgEl = null
      ...
      // For raster, we map to pixel coordinates of the original image
      const nx = (xInEl - offsetX) / renderW
      const ny = (yInEl - offsetY) / renderH
      return { x: nx * nw, y: ny * nh }
    }
  ...
  function zoom(factor, centerPoint = null) {
    ...
    // maintain relative position of centerPoint
    const ratioX = (cx - x) / w
    const ratioY = (cy - y) / h
    const nx = cx - ratioX * newW
    const ny = cy - ratioY * newH
  ...
  useEffect(() => {
    ...
    const onWheel = (e) => {
      ...
      const pt = clientToUnits(e.clientX, e.clientY)
      zoom(factor, pt)
    }
    el.addEventListener('wheel', onWheel, { passive: false })
    return () => el.removeEventListener('wheel', onWheel)
  }, [planRef, image, svgViewBox, disabled, clientToUnits])
  ...
  useEffect(() => {
    ...
    if (!el) return
    if (!(image && image.type === 'svg')) return
    let pinch = { active: false, startDist: 0, startVb: null, center: { x: 0, y: 0 } }
    ...
    const onTouchMove = (e) => {
      if (pinch.active && e.touches.length === 2 && pinch.startVb) {
        ...
        // pinch center in world units (pinch.center) should remain at same relative position in new viewbox
        const rx = (pinch.center.x - pinch.startVb.x) / pinch.startVb.w
        const ry = (pinch.center.y - pinch.startVb.y) / pinch.startVb.h
        const nx = pinch.center.x - rx * newW
        const ny = pinch.center.y - ry * newH
        setSvgViewBox({ x: nx, y: ny, w: newW, h: newH })
  ...
  return { svgViewBox, setSvgViewBox, zoom, pan, reset, clientToUnits, normToPercent }
}
